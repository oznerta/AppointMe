...
// Step 1 Component
const Step1: React.FC<{ next: () => void, setEmail: React.Dispatch<React.SetStateAction<string>>, setPassword: React.Dispatch<React.SetStateAction<string>>, setConfirmPassword: React.Dispatch<React.SetStateAction<string>>, errorMessage: string | null }> = ({ next, setEmail, setPassword, setConfirmPassword, errorMessage }) => (
  <>
    ...
    <Button onClick={next} className="mt-4 bg-blue-600 text-white hover:bg-blue-700">Next</Button>
  </>
);

// Step 2 Component
const Step2: React.FC<{ next: () => void, prev: () => void, setBrandName: React.Dispatch<React.SetStateAction<string>>, setFullName: React.Dispatch<React.SetStateAction<string>>, setContactNumber: React.Dispatch<React.SetStateAction<string>>, setServiceDescription: React.Dispatch<React.SetStateAction<string>>, userType: string, setUserType: React.Dispatch<React.SetStateAction<string>>,errorMessage: string | null }> = ({ next, prev, setBrandName, setFullName, setContactNumber, setServiceDescription, setUserType, errorMessage }) => (
  <>
    ...
    <div className="flex justify-between mt-4">
      <Button onClick={prev} className="bg-gray-400 text-white hover:bg-gray-500">Back</Button>
      <Button onClick={next} className="bg-blue-600 text-white hover:bg-blue-700">Next</Button>
    </div>
  </>
);

// Step 3 Component
const Step3: React.FC<{ submit: (e: React.FormEvent<HTMLFormElement>) => void, prev: () => void, handleFileChange: (e: React.ChangeEvent<HTMLInputElement>, setFile: React.Dispatch<React.SetStateAction<File | null>>) => void, setBrandLogo: React.Dispatch<React.SetStateAction<File | null>>, setBusinessLicense: React.Dispatch<React.SetStateAction<File | null>>, setIdFile: React.Dispatch<React.SetStateAction<File | null>>, setSelfie: React.Dispatch<React.SetStateAction<File | null>>, userType: string, errorMessage: string | null }> = ({ submit, prev, handleFileChange, setBrandLogo, setBusinessLicense, setIdFile, setSelfie, userType, errorMessage }) => (
  <form onSubmit={submit}>
    ...
    <div className="flex justify-between">
      <Button onClick={prev} className="bg-gray-400 text-white hover:bg-gray-500">Back</Button>
      <Button type="submit" className="bg-blue-600 text-white hover:bg-blue-700">Create Account</Button>
    </div>
  </form>
);
...
export function SignUpPage() {
  ...
  const handleSignUp = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
        ...
        if (brandLogo) {
            ...
            console.log("Brand logo uploaded and URL fetched:", merchantData.brandLogo);
        }
        ...
        if (businessLicense && userType === "business") {
            ...
            console.log("Business license uploaded and URL fetched:", merchantData.businessLicense);
        }
  ...
  const next = () => {
    if (step === 1) {
      ...
      // Validate Step 2
      if (!brandName || !fullName || !contactNumber || !serviceDescription || !userType) {
        setErrorMessage("Please fill in all fields.");
        return;
      }
      setErrorMessage(null);
  ...
  return (
    <div className="flex items-center justify-center h-screen">
      <div className='flex flex-col w-[350px]'>
        ...
        {step === 1 && <Step1 next={next} setEmail={setEmail} setPassword={setPassword} setConfirmPassword={setConfirmPassword} errorMessage={errorMessage} />}
        {step === 2 && <Step2 next={next} prev={prev} setBrandName={setBrandName} setFullName={setFullName} setContactNumber={setContactNumber} setServiceDescription={setServiceDescription} userType={userType} setUserType={setUserType} errorMessage={errorMessage} />}
        {step === 3 && <Step3 submit={handleSignUp} prev={prev} handleFileChange={handleFileChange} setBrandLogo={setBrandLogo} setBusinessLicense={setBusinessLicense} setIdFile={setIdFile} setSelfie={setSelfie} userType={userType} errorMessage={errorMessage} />}
        <p className="text-center mt-4 text-sm">
          Already have an account? <Link to="/signin" className="text-text-500">Sign In</Link>
        </p>
      </div>
    </div>
  );
